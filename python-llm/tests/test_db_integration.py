...
class TestArticleRepositoryIntegration:
    ...
    async def test_update_status(self, db_pool, repo, sample_article_id):
        ...
        await repo.update_status(sample_article_id, ArticleUpdate(status=ArticleStatus.PROCESSING))
        
        async with db_pool.acquire() as conn:
            updated = await conn.fetchrow("SELECT status FROM articles WHERE id = $1", sample_article_id)
    ...
    async def test_increment_retry(self, db_pool, repo, sample_article_id):
        ...
        async with db_pool.acquire() as conn:
            updated = await conn.fetchrow("SELECT status, retry_count FROM articles WHERE id = $1", sample_article_id)
        ...
        async with db_pool.acquire() as conn:
            updated_failed = await conn.fetchrow("SELECT status FROM articles WHERE id = $1", sample_article_id)
