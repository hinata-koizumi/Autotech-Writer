...
async def process_article(
    ...
    logger.info(f"Processing article {article_id}: {article['title']} (Status: {article.get('status')})")

    try:
        if _is_resuming(article):
            article_response = _resume_from_partial_failure(article)
        else:
            article_response, hook_text = await _prepare_new_content(article, repo, llm_service, config, pdf_service=pdf_service)
        ...
        logger.error(f"Error processing article {article_id}: {type(e).__name__}: {e}", exc_info=True)
...
def _resume_from_partial_failure(article: Dict[str, Any]) -> ArticleResponse:
    logger.info(f"Resuming article {article['id']} from {ArticleStatus.PARTIAL_FAILED.value}")
    return ArticleResponse(content=article["article_body"])


async def _mark_as_posting(article_id: int, response: ArticleResponse, hook_text: str, repo: ArticleRepository):
...
async def _prepare_new_content(
    ...
    # ArXiv PDF Fallback
    if article["source_type"] == "arxiv" and not article.get("full_content") and pdf_service:
        metadata = json.loads(article.get("metadata", "{}")) if isinstance(article.get("metadata"), str) else article.get("metadata", {})
...
async def _handle_x_post_translation(article: Dict[str, Any], llm_service: LLMService) -> tuple[ArticleResponse, str]:
...
async def _triage_step(article: Dict[str, Any], llm_service: LLMService, repo: ArticleRepository) -> bool:
    ...
    if not is_valuable:
        ...
        await repo.update_status(article["id"], ArticleUpdate(status=ArticleStatus.IGNORED))
        return False
    return True


async def _extraction_step(
    article: Dict[str, Any], repo: ArticleRepository, llm_service: LLMService, config: Config
    ...
    if not _is_extraction_valid(article_id, extracted_facts, config):
        await repo.update_status(article_id, ArticleUpdate(status=ArticleStatus.IGNORED))
        return None
...
async def _generation_step(article_id: int, extracted_facts: ExtractionResult, llm_service: LLMService) -> tuple[ArticleResponse, str]:
    ...
    hook_text = extracted_facts.problem_statement[:200] if extracted_facts.problem_statement else ""
    return article_response, hook_text


def _is_extraction_valid(article_id: int, extracted: ExtractionResult, config: Config) -> bool:
    """Check if extracted facts meet quality bars."""
    if not extracted.is_information_sufficient:
        logger.info(f"Article {article_id} ignored due to insufficient information. Reason: {extracted.reason_for_insufficient}")
        return False

    if extracted.extraction_confidence < 0.7:
        logger.info(f"Article {article_id} ignored due to low extraction confidence ({extracted.extraction_confidence}).")
        return False
    
    return True


async def _check_article_compliance(article_id: int, extracted: ExtractionResult, repo: ArticleRepository, config: Config) -> bool:
    ...
    if not is_clean:
        logger.warning(f"Article {article_id} rejected after extraction due to compliance. Words: {detected_words}")
        await repo.update_status(article_id, ArticleUpdate(status=ArticleStatus.REJECTED_COMPLIANCE))
        return False
    return True


def _resolve_posting_params(article: Dict[str, Any]) -> tuple[list[str], Optional[str], int]:
...
async def _posting_step(
    ...
    try:
        ...
        await repo.update_status(article_id, ArticleUpdate(status=ArticleStatus.COMPLETED))
        logger.info(f"Successfully processed and posted article {article_id} as thread.")
    except Exception as e:
        logger.error(f"X API error during posting for article {article_id}: {e}")
        await repo.update_status(article_id, ArticleUpdate(status=ArticleStatus.PARTIAL_FAILED))
        raise


async def _handle_processing_error(article: Dict[str, Any], repo: ArticleRepository, config: Config) -> None:
